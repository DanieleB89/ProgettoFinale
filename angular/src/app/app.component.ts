...
/*
  ...
  constructor(private http:HttpClient) {
    ...
    this.service.getAllCourses().subscribe(cs => {
      this.lista = cs;
  ...
  save(corsoForm: NgForm) {
    ...
    console.log(JSON.stringify(this.corso));
    this.service.saveCourse(this.corso).subscribe(
      c => console.log("Salvato corso")
    );
}

selection(item:Corso) {
  this.corso=item;
}

delete() {
  this.service.deleteCourse(this.corso).subscribe(c => console.log("Salvato corso"));
}


update() {
  this.service.updateCourse(this.corso).subscribe();
}*/
}
